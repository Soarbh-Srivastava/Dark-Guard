...
for (let resolution in icons_default) {
  icons_disabled[resolution] = `${icons_default[resolution].slice(
    0,
    -4
  )}_grey.png`;
}
...
brw.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) {
  ...
  if (
    tab.url.toLowerCase().startsWith("http://") ||
...
// Use the Google Safe Browsing API to check if a URL is safe
async function isUrlSafe(url) {
  try {
    const apiKey = "AIzaSyChH-qvUZaGhhJu9YuxdwqX0AqMaMC1l_U"; // Replace 'YOUR_API_KEY' with your actual API key
    const apiEndpoint =
      "https://safebrowsing.googleapis.com/v4/threatMatches:find?key=" + apiKey;

    const requestBody = {
      client: {
        clientId: "smiling-rhythm-393511",
        clientVersion: "1.0.0",
      },
      threatInfo: {
        threatTypes: [
    ...
    return !data.hasOwnProperty("matches");
  } catch (error) {
...
// Initialize status on extension start
chrome.runtime.onInstalled.addListener(function () {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs && tabs.length > 0) {
      sendUrlSafetyStatus(tabs[0].id);
    }
  });
});
...
// Add event listeners for tab updates
chrome.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) {
  sendUrlSafetyStatus(tabId);
});

chrome.tabs.onActivated.addListener(function (activeInfo) {
  sendUrlSafetyStatus(activeInfo.tabId);
});

// Add a function to send the URL safety status to the popup
function sendUrlSafetyStatus(tabId) {
  chrome.tabs.get(tabId, (tab) => {
    if (!tab || !tab.url) return;

    isUrlSafe(tab.url)
      .then((safe) => {
        chrome.runtime.sendMessage({ action: "updateStatus", status: safe });
      })
      .catch((error) => {
        console.error("Error checking URL safety:", error);
      });
