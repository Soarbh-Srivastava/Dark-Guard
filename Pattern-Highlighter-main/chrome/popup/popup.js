...
export class ExtensionPopup extends LitElement {
  ...
  async handleMessage(message, sender, sendResponse) {
    ...
    if ("countVisible" in message) {
      ...
      if (
        sender.tab.active &&
        (await getCurrentTab()).windowId === sender.tab.windowId
      ) {
  ...
  async firstUpdated() {
    ...
    if (
      currentTab.url.toLowerCase().startsWith("http://") ||
      currentTab.url.toLowerCase().startsWith("https://")
    ) {
      // Load the activation state.
      let currentTabActivation = await brw.runtime.sendMessage({
      ...
      if (currentTabActivation.isEnabled) {
        ...
        while (true) {
          try {
            ...
            this.results = await brw.tabs.sendMessage(currentTab.id, {
              action: "getPatternCount",
            });
  ...
  render() {
    return html`
      ...
      ></on-off-switch>
      <refresh-button
        .hide=${this.activation === this.initActivation}
        .app=${this}
      ></refresh-button>
      <redo-button .activation=${this.initActivation}></redo-button>
      <found-patterns-list
        .activation=${this.initActivation}
  ...
  render() {
    return html`
      ...
      ${!constants.patternConfigIsValid
        ? html`<h3>
            ${brw.i18n.getMessage("errorInvalidConfig")}
            <h3></h3>
          </h3>`
        : html``}
    `;
...
export class OnOffSwitch extends LitElement {
  ...
  // CSS styles for the HTML elements in the component.
  static styles = [sharedStyles, onOffSwitchStyles];
...
export class RefreshButton extends LitElement {
  ...
  // CSS styles for the HTML elements in the component.
  static styles = [sharedStyles, actionButtonStyles];
  ...
  async refreshTab() {
    ...
    await brw.runtime.sendMessage({
      enableExtension: this.app.activation === activationState.On,
  ...
  render() {
    ...
    return html`
      <div>
        <span @click=${this.refreshTab}
          >${brw.i18n.getMessage("buttonReloadPageForChange")}</span
        >
      </div>
    `;
  }
...
export class RedoButton extends LitElement {
  ...
  async redoPatternCheck(event) {
    await brw.tabs.sendMessage((await getCurrentTab()).id, {
      action: "redoPatternHighlighting",
    });
  }
  ...
  render() {
    ...
    return html`
      <div>
        <span @click=${this.redoPatternCheck}
          >${brw.i18n.getMessage("buttonRedoPatternCheck")}</span
        >
      </div>
    `;
  }
...
export class FoundPatternsList extends LitElement {
  ...
  // CSS styles for the HTML elements in the component.
  static styles = [sharedStyles, patternsListStyles, patternLinkStyles];
  ...
  render() {
    ...
    return html`
      <div>
        ...
        <h2 style="color: ${this.results.countVisible ? "red" : "green"}">
          ${this.results.countVisible}
        </h2>
        <ul>
          ${this.results.patterns?.map((pattern) => {
            let currentPatternInfo = constants.patternConfig.patterns.find(
              (p) => p.name === pattern.name
            );
            if (pattern.elementsVisible.length === 0) {
              return html``;
            }
            return html` <li title="${currentPatternInfo.info}">
              <a href="${currentPatternInfo.infoUrl}" target="_blank"
                >${pattern.name}</a
              >: ${pattern.elementsVisible.length}
            </li>`;
          })}
        </ul>
      </div>
...
export class ShowPatternButtons extends LitElement {
  ...
  async showPattern(step) {
    ...
    await brw.tabs.sendMessage((await getCurrentTab()).id, {
      showElement: this._currentPatternId,
    });
  }
  ...
  getCurrentPatternText() {
    ...
    if (this._currentPatternId) {
      ...
      if (idx !== -1) {
        ...
        let currentPatternInfo = constants.patternConfig.patterns.find(
          (p) => p.name === this._visiblePatterns[idx].patternName
        );
        // Generate the HTML text.
        return html` <h3 title="${currentPatternInfo.info}">
          <a href="${currentPatternInfo.infoUrl}" target="_blank"
            >${this._visiblePatterns[idx].patternName}</a
          >
        </h3>`;
      }
    }
  ...
  render() {
    ...
    return html`
      <div>
        ...
        <span
          >${brw.i18n.getMessage("showPatternState", [
            this.getCurrentPatternNumber(),
            this.results.countVisible.toString(),
          ])}</span
        >
  ...
  render() {
    return html`
      <div>
        ...
        <ul>
          ${constants.patternConfig.patterns.map(
            (pattern) =>
              html` <li title="${pattern.info}">
                <a href="${pattern.infoUrl}" target="_blank">
                  ${pattern.name}
                  (${pattern.languages.map((l) => l.toUpperCase()).join(", ")})
                </a>
              </li>`
          )}
        </ul>
